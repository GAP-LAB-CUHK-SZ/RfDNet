...
class Encoder(nn.Module):
    def __init__(self, num_inputs, in_channel=3):
        super(Encoder, self).__init__()
        self.num_inputs = num_inputs
        self.conv1 = torch.nn.Conv2d(1, 128, (1, in_channel))
...
class Autoencoder(nn.Module):
    def __init__(self, num_inputs, num_coarses, num_fines, grid_size, in_channel=3):
        super(Autoencoder, self).__init__()
        self.num_inputs = num_inputs
        self.num_coarses = num_coarses
        self.num_fines = num_fines
        self.grid_size = grid_size
        self.Encoder = Encoder(num_inputs, in_channel)
    ...
    def forward(self, x):
        ...
        coarses = F.relu(self.fc1(feature))
        coarses = self.fc2(coarses)
        coarses = coarses.reshape(-1, self.num_coarses, 3)

        batch_size = x.size()[0]
        grid = torch.meshgrid(
            [torch.linspace(-0.05, 0.05, self.grid_size), torch.linspace(-0.05, 0.05, self.grid_size)])
